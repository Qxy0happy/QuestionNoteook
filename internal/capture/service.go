...
// NewService 用一个已经开好的题图 Store 与题库构造服务。
func NewService(store *Store, lib *library.Service) *Service {
	return &Service{store: store, library: lib}
}
...
func (s *Service) Capture(frameBase64 string, quad Quad) (library.Question, error) {
	raw, err := base64.StdEncoding.DecodeString(frameBase64)
	if err != nil {
		return library.Question{}, fmt.Errorf("采集: 帧不是合法的 base64: %w", err)
	}

	img, _, err := image.Decode(bytes.NewReader(raw))
	if err != nil {
		return library.Question{}, fmt.Errorf("采集: 帧解不出图像: %w", err)
	}

	card, err := Rectify(img, quad)
	if err != nil {
		return library.Question{}, err
	}
