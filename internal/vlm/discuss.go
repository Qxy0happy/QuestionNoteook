...
//
// 第 6 条（公式的定界符）不是暂定的：它对应的是「公式渲染不对」这条实打实的故障。
// 回答会被当成 markdown 交给 marked + KaTeX 渲染，而那条链只认 $ / $$ 两种定界符；
// 模型按 LaTeX 的老习惯吐 \[ ... \]，到了界面上就是一段原样的文本 —— 实测过的原文
// 就长这样（见 Markdown.svelte 里那段注释与票据里的设备截图）。
// 界面那一侧同时也做了归一（认得 \[ ... \]，不会再原样漏出来），两边都做是有意的：
// 提示词让模型**一开始就吐对的**，界面兜住**已经吐错的那一种** —— 历史记录里的、
// 以及模型偶尔不听话的。只有一边的话，任何一次走偏都还是坏在用户眼前。
//
...
5. 篇幅跟着问题走：一句能说清就一句话。
6. 公式一律用 $ ... $（夹在句子里的）与 $$ ... $$（独立成行的）这两种写法包起来。
   不要用 \[ ... \] 与 \( ... \)：这个本子的界面只认带 $ 的那种，另一种在他眼里
   就是一段原样的文本。独立成行的公式把 $$ 与公式写在**同一行**里，像
   $$ A\alpha = [3,3,3]^T = 3\alpha $$ 这样，别让 $$ 自己单占一行。`
